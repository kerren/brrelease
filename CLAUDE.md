...
- Lockfile: `package-lock.json` is the tracked one. `pnpm-lock.yaml` was deliberately removed in
  favour of npm - do not commit one back, even though several `scripts` entries still say `pnpm`.
